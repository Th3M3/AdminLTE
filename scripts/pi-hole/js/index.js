...
var customTooltips = function(tooltip) {
  ...
  tooltipEl.style.top =
    position.top + tooltip.caretY + window.pageYOffset + "px";
...
function updateClientsOverTime() {
  $.getJSON("api.php?overTimeDataClients&getClientNames", function(data) {
    ...
    for (
      i = clientsChart.data.datasets.length;
      plotdata.length && i < plotdata[0].length;
      i++
    ) {
...
function updateTopClientsChart() {
  $.getJSON("api.php?summaryRaw&getQuerySources&topClientsBlocked", function(
    ...
    for (client in data.top_sources_blocked) {
      if (
        Object.prototype.hasOwnProperty.call(data.top_sources_blocked, client)
      ) {
        // Sanitize client
        if (escapeHtml(client) !== client) {
          // Make a copy with the escaped index if necessary
          data.top_sources_blocked[escapeHtml(client)] =
        ...
        url =
          ...
          "</a>";
        percentage =
          (data.top_sources_blocked[client] / data.ads_blocked_today) * 100;
...
function updateSummaryData(runOnce) {
  ...
  $.getJSON("api.php?summary", function(data) {
    ...
    if ("FTLnotrunning" in data) {
      ...
      // Adjust text
      $("#temperature").html(
        '<i class="fa fa-circle text-red"></i> FTL offline'
      );
      // Show spinner
