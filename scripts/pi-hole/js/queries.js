...
function handleAjaxError(xhr, textStatus) {
  if (textStatus === "timeout") {
    ...
    alert(
      "An error occured while loading the data: Connection refused. Is FTL running?"
    );
  } else {
    alert(
  ...
  tableApi = $("#all-queries").DataTable({
    rowCallback: function(row, data) {
      ...
      if (data.length > 7 && data[7] > 0) {
        ...
        $("td:eq(5)", row).html(
          content + " (" + (0.1 * data[7]).toFixed(1) + "ms)"
        );
      }
    },
    dom:
    ...
    initComplete: function() {
      ...
      api.$("td:eq(2)").hover(
        function() {
          if (autofilter()) {
            this.title =
              "Click to show only queries with domain " + this.textContent;
            this.style.color = "#72afd2";
          } else {
            this.title = "";
            this.style.color = "";
      ...
      api.$("td:eq(3)").hover(
        function() {
          if (autofilter()) {
            this.title =
              "Click to show only queries made by " + this.textContent;
            this.style.color = "#72afd2";
          } else {
            this.title = "";
            this.style.color = "";
